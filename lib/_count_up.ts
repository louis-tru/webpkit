...
export interface CountUpOptions { // (default)
	...
	easingFn?: (t: number, b: number, c: number, d: number) => number; // easing function for animation (easeOutExpo)
...
export class CountUp {
	...
	private finalEndVal: number = 0; // for smart easing
	...
	constructor (
		...
		this.el = (typeof target === 'string') ? document.getElementById(target) as HTMLElement : target;

		if (this.el) {
			this.printValue(this.startVal);
		} else {
			throw Error.new(this.error = '[CountUp] target is null or undefined');
		}
	}
	...
	formatNumber = (num: number): string => {
		...
		if (this.options.numerals && this.options.numerals.length) {
			var numerals = this.options.numerals;
