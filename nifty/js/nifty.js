...
! function ($) {
	...
	var pageHolder, floatContainer = {},
		notyContainer, niftyContainer, niftyContentContainer, addNew = false,
...
}(jQuery, typeof global == 'object' ? global : window);
...
! function ($) {
	...
	var niftyAside = null,
		niftyContainer, niftyWindow = $(window),
		asideMethods = {
		...
		boxedContainer = $('#container').children('.boxed'),
		navlg = 0,
		navsm = 0,
...
! function ($) {
	...
	var niftyMainNav, niftyAside, niftyContainer, niftyMainnavScroll, niftyAsideScroll, updateScrollInterval,
		...
		bindAffix = function () {
			...
			if (niftyMainNav.length) niftyMainNav.niftyAffix({
				className: 'mainnav-fixed'
			});
			if (niftyAside.length) niftyAside.niftyAffix({
